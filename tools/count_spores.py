...
flags = tf.app.flags
flags.DEFINE_string('image_dir', '/home/plantvillage/Dropbox/Object_Detection/spore/data/validation_images/23/23_originals', 'Path to the parent directory of the project')
flags.DEFINE_string('output_dir', '/home/plantvillage/Dropbox/Object_Detection/spore/data/validation_images/23/23.0_counts_val_0.5', 'Path to the parent directory of the project')
flags.DEFINE_string('model_path','/home/plantvillage/Dropbox/Object_Detection/spore/model/pb_files/spore_detect_23.0.pb','The full file path to the model')
flags.DEFINE_string('labels_path','/home/plantvillage/Dropbox/Object_Detection/spore/data/spore_label_map.pbtxt','The full file path to the labels file')
flags.DEFINE_string('gt_file','/home/plantvillage/Dropbox/Object_Detection/spore/counts/counts_23_val.txt', 'The text file containing the filenumbers and groundtruth counts')
...
MODEL_VERSION = MODEL_NAME[(len(MODEL_NAME)-7):(len(MODEL_NAME)-3)]
...
gt_data = np.genfromtxt(gt_file_path, usecols=(1), skip_header=0, dtype=str)

print(gt_data)
# Concatenate a column onto the data array for predicted counts
num_data_pts = np.size(gt_data,0)
data = np.zeros((num_data_pts))
data[:] = gt_data
...
with detection_graph.as_default():
  with tf.Session(graph=detection_graph) as sess:
    ...
    i = 0
    for image_path in enumerate(sorted(os.listdir(PATH_TO_TEST_IMAGES_DIR))):
      ...
      counts_filename = os.path.join(PATH_TO_RESULTS_IMAGES_DIR, ('spore_counts_' + 'v' + MODEL_VERSION + '_' + str(score_threshold) + '%.txt'))
