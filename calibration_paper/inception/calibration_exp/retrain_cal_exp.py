...
import classifyCassava as classify
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
  ...
  for sub_dir in sub_dirs:
    ...
    for file_name in file_list:
      ...
      # the same leaf.
      hash_name = re.sub(r'_nohash_.*$', '', file_name) # look into function
...
def main(_):
  ...
  eval_set = []
  for label_name, label_lists in image_lists.items():
    for category in ['testing', 'validation']:
      eval_set += label_lists[category]
  print('eval_set')
  print(eval_set)
  ...
  for disease_class, disease_set in disease_classes.items():
    ...
    for file_name in disease_set:
      total_count += 1
      file_name = "/Users/singhcpt/dev/calibration_exp/data/images/" + disease_class + "/" + file_name
      ...
      for i in top_k:
        ...
        if results[i] > max_prob:
          max_prob = results[i]
          max_label = labels[i]
      print(max_label, max_prob, file_name)
    ...
    workbook = xw.Workbook('/Users/singhcpt/dev/calibration_exp/data/results_' + disease_class + '.xlsx')
  ...
  # Printing results into excel sheet    
  workbook = xw.Workbook('/Users/singhcpt/dev/calibration_exp/data/results_total.xlsx')
  worksheet = workbook.add_worksheet()
